...
class Puppy {
    ...
    constructor(
        img="https://www.puppyleaks.com/wp-content/uploads/2017/09/puppysmile.png",
        sound="WOOF!") {
            this.img = img;
            this.sound = sound;
        }
    ...
    // Render a Div that you can click on to bark
        render() {
            let puppyCard = $('<div>');
            puppyCard.attr('title',`puppo that says ${this.sound}`)
            puppyCard.attr('class', 'puppCard col-sm-4');
            puppyCard.attr('background-image', `url(${this.img})`)
            puppyCard.append(`<img src =${this.img} class = 'puppyCard'/>`);
...
addPuppy(img,sound) {
    ...
    render() {
        ...
        this.parentElement.append(this.puppyWrapper);
...
let app = new PuppyApp(
    $('#content'), 
    [{
        url: 'https://www.puppyleaks.com/wp-content/uploads/2017/09/puppysmile.png', 
        bark: "BARK BARK BARK"
    }]
)
